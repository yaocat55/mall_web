...
import qiniuApi from '@/api/tools/qiniu';

export default function StorageQiniu() {
  const crud = useCrud({
    title: '七牛云',
    url: 'api/qiNiuContent',
    crudMethod: qiniuApi,
    defaultForm: { id: null, name: '', accessKey: '', secretKey: '', bucket: '' },
  });
  ...
  return (
    <Box sx={{ p: 1 }}>
      ...
      >
        ...
        <TextField
          label="AccessKey"
          value={crud.form.accessKey || ''}
          onChange={(e) => crud.setForm({ ...crud.form, accessKey: e.target.value })}
          fullWidth
          margin="dense"
        />
        <TextField
          label="SecretKey"
          value={crud.form.secretKey || ''}
          onChange={(e) => crud.setForm({ ...crud.form, secretKey: e.target.value })}
          fullWidth
          margin="dense"
        />
        <TextField
          label="Bucket"
          value={crud.form.bucket || ''}
          onChange={(e) => crud.setForm({ ...crud.form, bucket: e.target.value })}
          fullWidth
          margin="dense"
        />
      </CrudDialog>
