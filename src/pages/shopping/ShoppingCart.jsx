...
import shoppingCartApi from '@/api/shopping/shoppingCart';
    ...
    <Box sx={{ p: 1 }}>
      ...
      <CrudDialog open={crud.dialogOpen} title={crud.dialogTitle}
        ...
        <TextField label="商品ID" value={crud.form.productId || ''}
          ...
          fullWidth margin="dense" size="small" />
        <TextField label="数量" value={crud.form.quantity || ''}
          onChange={(e) => crud.setForm({ ...crud.form, quantity: e.target.value })}
          fullWidth margin="dense" size="small" />
      </CrudDialog>
    </Box>
  );
